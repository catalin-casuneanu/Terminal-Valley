class TurnCycle {
  constructor({ battle, onNewEvent, onWinner }) {
    ...
    this.currentTeam = null; //or "enemy"
  }

  decideFirstTeam() {
    const playerId = this.battle.activeCombatants["player"];
    const enemyId = this.battle.activeCombatants["enemy"];

    const player = this.battle.combatants[playerId];
  ...
  async turn() {
    ...
    //Stop here if we are replacing this pokeLang
    ...
    if (targetDead) {
      ...
      if (submission.target.team === "enemy") {
        ...
        const playerActivepokeLangId = this.battle.activeCombatants.player;
        ...
        await this.onNewEvent({
          type: "giveXp",
          xp,
          combatant: this.battle.combatants[playerActivepokeLangId]
        })
      }
    }
